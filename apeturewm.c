...
static int numws = CFG_WORKSPACES;   /* active workspaces, configurable: 1..MAXWS */
...
static char barfontname[128] = CFG_BAR_FONT;
static char barleftcfg[128] = CFG_BAR_LEFT;
static char barcentercfg[128] = CFG_BAR_CENTER;
static char barrightcfg[128] = CFG_BAR_RIGHT;
static int screen_off_minutes = CFG_SCREEN_OFF_MIN;
static BarPosition barpos = CFG_BAR_POSITION;
static Display *dpy;
static Window root;
static int sw, sh;
...
static void setup_wm_check(void) {
    static const char name[] = "apeturewm";
...
/* Build the runtime tables from the compile-time config.h (dwm-style).
   Appearance/scalars are initialised at their declarations; this wires up the
   keybinds, window rules and autostart list. */
static void apply_config(void) {
    nmodbinds = 0;
    for (size_t i = 0; i < sizeof keybinds / sizeof keybinds[0]; i++) {
        KeyCode code = XKeysymToKeycode(dpy, keybinds[i].keysym);
        if (code) upsert_modbind(keybinds[i].mod, code, keybinds[i].action);
    }

    nrules = 0;
    for (size_t i = 0; i < sizeof config_rules / sizeof config_rules[0]; i++) {
        const Rule *r = &config_rules[i];
        if (!r->class_name[0] && !r->instance_name[0] && !r->title[0])
            continue;   /* empty match => sentinel / disabled, skip it */
    ...
    for (size_t i = 0; autostart[i]; i++) {
        if (nautostart >= autostart_limit) break;
        copystr(autostart_cmds[nautostart++], sizeof autostart_cmds[0], autostart[i]);
    }
}
...
static void restartwm(void) {
    ...
    /* drop our grabs/state so the new instance starts clean, then re-exec.
       argv[0] is normally "apeturewm", so execvp resolves it via PATH to the
...
static void setupbars(void) {
    ...
    for (int i = 0; i < nmons; i++) {
        ...
        XStoreName(dpy, m->barwin, "apeturewm-bar");
        hint.res_name = "apeturewm-bar";
        hint.res_class = "ApetureWMBar";
